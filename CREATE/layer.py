...
class Decoder1(nn.Module):
    def __init__(self, multi, channel5):
        super().__init__()
        self.multi = multi
        self.target_length = TARGET_LENGTH  # 9984
        if 'seq' in self.multi:
    ...
    def forward(self, out):
        ...
        if 'seq' in self.multi:
            x = self.convt1(out)
            x = x[:, :, :self.target_length]  # 强制裁剪到 9984
            decs.append(x)
        if 'open' in self.multi:
            x = self.convt11(out)
...
class split_quant(nn.Module):
    ...
    def forward(self, enc3):
        ...
        shape0, shape1, shape2 = z_e.shape
...
class create(nn.Module):
    def __init__(self, num_class=5, multi=['seq','open','loop'], channel1=512, channel2=384, channel3=128, channel4=200, channel5=200, embed_dim=128, n_embed=200, split=16, ema=True, e_loss_weight=0.25, mu=0.01):
        ...
        self.fc = torch.nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(self.seq_len * embed_dim, 1024),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(1024, 512),
            nn.ReLU(inplace=True),
    ...
    def _get_seq_len(self):
        channels = sum([4 if 'seq' in self.vqvae.multi else 0,
                        1 if 'open' in self.vqvae.multi else 0,
                        1 if 'loop' in self.vqvae.multi else 0])
        x = torch.zeros(1, channels, TARGET_LENGTH)  # 使用 9984
        with torch.no_grad():
            enc1 = self.vqvae.enc1(x)
            enc2 = self.vqvae.enc2(enc1)
            enc3 = self.vqvae.enc3(enc2)
            z_e = self.vqvae.quantize3.quantize_conv3(enc3)
            return z_e.shape[2]
